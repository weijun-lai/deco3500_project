  ...
  function storyPageClose(type,dx,dy) {
    ...
    .attr("transform", "translate(0," + dy*diameter*2 + ")");
  ...
  function sumbitStory() {
   clearAll();
    ...
    for (var user in data_user_json.users) {
        if (data_user_json.users[user].username == ProfileName) {
        data_user_json.users[user].VoteCase = VoteCase;
         data_user_json.users[user].LinksContributed = LinksContributed;
         data_user_json.users[user].TopticAdded = TopticAdded;
         data_user_json.users[user].level=UserLevel;
        updateUserpanel();
        break;
        }
    }
